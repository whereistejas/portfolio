...
3. **SSH commit signing is enabled.** Commands that create or modify commits require SSH agent access. Always use `required_permissions: ["all"]` when running jj commands that mutate commits (e.g. `split`, `describe`, `squash`, `new`, `abandon`, `rebase`).
...
`jj split` without arguments is interactive and will hang in agent environments. However, **non-interactive splitting works** by passing file paths as positional arguments with `-m`:

```bash
# Non-interactive split: move specific files into a new commit
jj split --revision @ path/to/file1 path/to/file2 -m "commit message"
```

This peels off a commit containing only the listed files and leaves everything else in the working copy. Repeat to create multiple logical commits from a single working copy.
...
| Split (non-interactive) | `jj split --revision @ <files> -m "msg"` |
...
Every commit created by or with help from a Cursor agent **must** include:

```
Co-authored-by: Cursor <cursoragent@cursor.com>
```

Add it as the last line of the commit message, separated by a blank line from the body. When using non-interactive split or describe, include it via heredoc:

```bash
jj split --revision @ src/file.ts -m "$(cat <<'EOF'
add new feature
...
Use `jj desc` to add the trailer to existing commits if it was missed.

### Package Manager

This project uses **Bun**, not npm. Use `bun run` for scripts.
...
6. **Always co-author**: Include the co-author trailer on every agent-assisted commit
7. **Use `-m` flags**: Never rely on editor prompts in agent environments
